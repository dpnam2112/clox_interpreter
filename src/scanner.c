...
/* check_keyword: Used to if the current lexeme is a keyword (var, and, or, .etc)
 *
 * Compare the part of the current lexeme starting from @start and the part of the 
 * keyword which also starts from @start.
 * */
static TokenType check_keyword(int start, int length, const char * part, TokenType type)
{
...
Token scan_token()
{
	...
	case '<': return match('=') ? make_token(TK_LESS_EQUAL) :
				  make_token(TK_LESS);
	case '>': return match('=') ? make_token(TK_GREATER_EQUAL) :
				  make_token(TK_GREATER);
	case '=': return match('=') ? make_token(TK_EQUAL_EQUAL) :
		  		make_token(TK_EQUAL);
	case '!': return match('=') ? make_token(TK_BANG_EQUAL) :
		  		make_token(TK_BANG);
