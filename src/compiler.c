 ...
 * @local: indicate whether the upvalue refer to an object belonging to the
 * enclosing closure/function
 * @long_offset: indicate whether the @index is one-byte long (i.e, 0 <= @index
 * <= UINT8_MAX).
 *
 ...
 * @param compiler: compiler that compiles the current function / closure
 * object
 * @param index: index of the upvalue in the enclosing function's array of
 * upvalues if local is false index of the value referenced by the upvalue in
 * the virtual machine's stack
 * @param local: indicate whether the value referenced is inside stack or
 * another function's upvalue array
...
static void parse_precedence(Precedence prec) {
  ...
  prefix_rule();  // call the appropriate function to parse the prefix
...
static uint16_t patch_jump(uint32_t jmp_param_pos) {
  // the position where the jump starts. We read the jump parameter before
  // starting to jump.
...
static void end_scope() {
  ...
  for (Local* local_it = &current->locals[current->local_count - 1];
    ...
    // so we need to add a pop instruction corresponding to each local
    // variable
...
static void for_stmt() {
  ...
  if (!check(TK_SEMICOLON)) {
    ...
    consume(TK_SEMICOLON,
            "Expect ';' after for-loop if there is no loop statement.");
...
static void binary() {
  ...
  switch (op.type) {
    /* We use chunk_append here because there are some cases where the operator
     * and its operands do not lie on the same line. If an error occurs, we
     * have to report the exact line on which a token is. */
    case TK_PLUS:
